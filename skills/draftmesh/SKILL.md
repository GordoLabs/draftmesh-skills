...
Look for MCP tools named `list_workspaces` and `read_doc`. If you have them,
skip straight to §3 — you're connected. If you don't, bootstrap below.
...
JSON entry (name the server `draftmesh`):
...
command = "npx"
args = ["-y", "draftmesh-mcp"]
env = { DRAFTMESH_AGENT_NAME = "Codex" }
```
...
DraftMesh daemon if one isn't already running; you don't need a separate
install or background process.
...
Windows with a spawn-related error, start the daemon by hand first —
`npm i -g draftmesh` then run `draftmesh` — and retry once it's up. Off
Windows this doesn't apply.
...
2. **Hand back the `uiUrl`.** Every write response carries one — give it to
   the person you're doing this for so they can see exactly what you changed
   or asked, in context, rather than describing it secondhand.
...
- **A tool call errors immediately, connection refused.** The daemon isn't
  running and couldn't auto-launch. See the Windows caveat in §2, or start it
  by hand: `npm i -g draftmesh && draftmesh`.
